...
export function LoadingPage({ pageTitle }: Props) {
  return (
    <div className="relative flex h-full animate-fade-in-delayed items-center justify-center rounded-xl bg-theme">
      <div className="flex flex-col items-center gap-y-6">
        <div className="relative h-32 w-32">
          {animationSquares.map((square) => (
            <div
              key={square.color}
              className={`absolute top-[calc(50%-12px)] left-[calc(50%-12px)] size-5 animate-expand-collapse rounded-xs ${square.color} ${square.position}`}
            />
          ))}
        </div>
        <div className="space-y-3 text-center">
          <p className="font-semibold text-gray-700 text-lg leading-tight">
            {pageTitle}を表示中...
          </p>
